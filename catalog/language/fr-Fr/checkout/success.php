<?php

// Heading

$_['heading_title']        = 'Votre commande a bien été reçue!';


// Text

$_['text_basket']          = 'Chariot';
$_['text_checkout']        = 'Acheter';
$_['text_success']         = 'Succès';
$_['text_customer']        = ' <p> Votre commande a été traitée avec succès! </p>  <p> Vous pouvez voir l`historique de vos commandes en allant dans <a href="%s"> Mon compte </a> page et en cliquant sur <a href="%s"> historique </a> . </p>  <p> Si votre achat est associé à un téléchargement, vous pouvez accéder au compte <a href="%s"> téléchargements </a> page pour les voir. </p>  <p> Veuillez adresser toutes les questions que vous avez à <a href="%s"> propriétaire du magasin </a> . </p>  <p> Merci de magasiner avec nous en ligne! </p> 
';
$_['text_guest']           = ' <p> Votre commande a été traitée avec succès! </p>  <p> Veuillez adresser toutes les questions que vous avez à <a href="%s"> O`Toit </a> . </p>  <p> Merci de magasiner avec nous en ligne! </p> ';




					/* 
					  Copyright (C) 2016-2017 Apptiko.
					  Created by Apptiko - http://extension.apptiko.com.
					  Time-stamp-code:Sat1117071209					  
					*/
